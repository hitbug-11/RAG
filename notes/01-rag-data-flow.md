...
**第五步：Context Packing 组织上下文**

系统根据 Token 预算选择、去重、截断并排列候选 Chunk，同时保留引用标识。此步骤的输出是 `Packed Context`：
...
Context Packing 只负责整理证据，还没有形成 LLM 的完整输入。

**第六步：Prompt Builder 构造 Prompt**

Prompt Builder 将系统指令、Packed Context 和原始用户问题组合成 `Final Prompt`：
...
注意，LLM 接收的是整理后的自然语言上下文，而不是 `[0.021, -0.184, ...]` 这样的检索向量。用于检索的 Retrieval Query 即使经过改写，Prompt 中通常仍要保留原始用户问题。

**第七步：Generator 生成答案**

Generator 读取 Final Prompt，生成答案草稿或在证据不足时拒答：
...
系统利用 Top-k Chunks 的 Metadata 映射来源，并完成格式化和必要的安全检查，得到最终响应：
